...
- **Licence**: Kaption is dual-licensed under AGPL-3.0 and a commercial
  licence (see [`LICENSE`](./LICENSE)). **By opening a pull request you
  agree that your contribution is licensed under both options
  (inbound = outbound).** This means we can keep offering Kaption under
  the same dual licence, with your contribution included, without
  needing to come back and ask for permission later. We do not require
  a signed CLA — opening the PR is the agreement.
...
- **Trademarks**: "Kaption" and the icon are trademarks of the project
  author. Source rights are not trademark rights. If you fork, give
  your fork a different name. See [`TRADEMARKS.md`](./TRADEMARKS.md).
...
- New locales for the UI — add keys to
  ...
  are actively maintained, but new locales are welcome.
...
1. **Fork.** Create a branch off `master` with a short, kebab-case
   name (`fix-dashboard-badge`, `hsr-tuning-v33`).
2. **Build the solution.** You need the .NET 10 SDK (10.0.203 or newer)
   on Windows:
...
3. **Run the tests:**
   ...
   Expected on a clean checkout: 190 pass, 2 pre-existing
   data-dependent fails (`DialoguePredictionTests`), 5 external-data
   skips.
4. **Verify user-visible changes manually.** WPF overlay behaviour is
   genuinely hard to unit-test. If you changed layout code, attach
   before / after screenshots to the PR.
5. **Keep commits focused.** Prefer short imperative subjects with a
   scope first:
   - `Dashboard: fix disabled-state badge not clearing`
   - `HSR: bump stability window for typewriter dialogue`
6. **Open the PR against `master`.** Fill out the description — one
...
not yet ship, please open an issue first — there is significant
infrastructure on our side (LLM keys, multi-hour runs, publishing
scripts) and we want to coordinate before you spend time.
